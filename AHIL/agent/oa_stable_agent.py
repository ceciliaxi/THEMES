...
class OAStableAgent(BaseAgent):
    def __init__(self,
        ...
        self.args = ArgParse({
            'env'           : env, # REVISED HERE: <-- self.env.unwrapped.spec.id
            'algo'          : self.algorithm                        ,
            'folder'        : self.base_path + 'contrib/baselines_zoo/trained_agents', ## REVISED HERE
            'n_timesteps'   : 1000                                  ,
            'n_envs'        : 1                                     ,
            'exp_id'        : -1                                    ,
            'verbose'       : 1                                     ,
    ...
    # Load the pre-trained expert's model
    ...
    # Set the parameters for loading pretrained model
    def _set_vars(self):
        for env_module in self.args.gym_packages:
            importlib.import_module(env_module)

        # Access to the folder with pre-trained models
        ...
        # Path to the model of current environment
